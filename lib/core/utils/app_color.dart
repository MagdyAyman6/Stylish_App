import 'package:flutter/material.dart';

abstract class AppColor {
  static const kPrimaryColor = Color(0xffF83758);
  static const kBlackColor = Color(0xff000000);
  static const kWhiteColor = Color(0xffFFFFFF);
  static const kGreyColor = Color(0xffbbbbbb);
  static const kHintColor = Color(0xff676767);
  static const kIconFormField = Color(0xff626262);
  static const kSubtitleColor = Color(0xffA8A8A9);
  static const kIndicatorColor = Color(0xff17223B);
  static const kIndicator2Color = Color(0xffFFA3B3);
  static const kFormFieldBackground = Color(0xffF3F3F3);
  static const kFormFieldBorderColor = Color(0xffA8A8A9);
  static const kSubtitle2Color = Color(0xffF2F2F2);
  static const kStylishColor = Color(0xff4392F9);
  static const kIconFeatureColor = Color(0xff232327);
  static const kTitleFeatureColor = Color(0xff21003D);
  static const kStarRatingColor = Color(0xffEDB310);
  static const kReviewCountColor = Color(0xffA4A9B3);
  static const kTrendingContainer = Color(0xffFD6E87);
  static const kHomeRewardCover = Color(0xffE7E7EB);
// static const Color primaryColor = Color(0xFF6200EA);
// static const Color primaryLight = Color(0xFFBB86FC);
// static const Color primaryDark = Color(0xFF3700B3);
//
// static const Color secondaryColor = Color(0xFF03DAC6);
// static const Color secondaryLight = Color(0xFF66FFF9);
// static const Color secondaryDark = Color(0xFF00A896);
//
// static const Color textPrimary = Color(0xFF000000);
// static const Color textSecondary = Color(0xFF757575);
// static const Color textHint = Color(0xFFBDBDBD);
// static const Color textError = Color(0xFFD32F2F);
//
// static const Color backgroundColor = Color(0xFFFFFFFF);
// static const Color surfaceColor = Color(0xFFF5F5F5);
//
// static const Color errorColor = Color(0xFFB00020);
// static const Color successColor = Color(0xFF388E3C);
// static const Color warningColor = Color(0xFFFFC107);
}
